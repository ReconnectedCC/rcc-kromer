...
public class TransactionsCommand {
    public static void register(CommandDispatcher<ServerCommandSource> dispatcher, CommandRegistryAccess registryAccess, CommandManager.RegistrationEnvironment environment) {
        dispatcher.register(literal("transactions")
    ...
    public static int checkTransactions(CommandContext<ServerCommandSource> context) throws CommandSyntaxException {
        ...
        if (!Kromer.kromerStatus) {
            source.sendFeedback(() -> Locale.use(Locale.Messages.KROMER_UNAVAILABLE), false);
            return 0; // Return failure
        }
        HttpRequest request;
        try {
            request = HttpRequest.newBuilder()
                    .uri(new java.net.URI(Kromer.config.KromerURL() + "api/krist/addresses/" + address + "/transactions/?limit=10&offset=" + (page-1)*10))
                    .GET()
                    .build();
        ...
        Kromer.httpclient.sendAsync(request, HttpResponse.BodyHandlers.ofString()).whenComplete((response, throwable) -> {
            ...
            context.getSource().sendFeedback(() -> Text.literal("<green>Transactions for " + player.getName().getString() + " on page " + finalPage +":<reset>"), false);
            if (responseObj.transactions.isEmpty()) {
                context.getSource().sendFeedback(() -> Text.literal("<red>No transactions found<reset>"), false);
                ...
                for (var transaction : responseObj.transactions) {
                    ...
                    context.getSource().sendFeedback(() -> TextParserUtils.formatText(transactionText), false);
                }
            }
            boolean hasNextPage = responseObj.count >= 10;
            boolean hasPreviousPage = finalPage > 1;
            StringBuilder navigationText = new StringBuilder("<green>Navigation: ");
            if (hasPreviousPage) {
                navigationText.append("<run_cmd:'/transactions ").append(finalPage - 1).append("'><aqua>[Prev. Page]</aqua></run_cmd> ");
            }
            if (hasNextPage) {
                navigationText.append("<run_cmd:'/transactions ").append(finalPage + 1).append("'><aqua>[Next Page]</aqua></run_cmd>");
            }
            context.getSource().sendFeedback(() -> TextParserUtils.formatText(navigationText.toString()), false);
