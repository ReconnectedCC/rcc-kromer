...
public class API {
    ...
    public static CompletableFuture<HttpResponse<String>> startWs() throws URISyntaxException {
        ...
        return Kromer.httpclient.sendAsync(request, HttpResponse.BodyHandlers.ofString());
    }
    public static boolean apiErrorHandler(CommandContext<ServerCommandSource> context, Throwable throwable, HttpResponse<String> response) {
        if (throwable != null) {
    ...
    public static CompletableFuture<GetAddressResponse> getBalance(String address, CommandContext<ServerCommandSource> context) {
        var url = String.format(Kromer.config.KromerURL() + "api/krist/addresses/%s", address);
        HttpRequest request;
        try {
        ...
        CompletableFuture<GetAddressResponse> completableFuture = new CompletableFuture<>();
        ...
        Kromer.httpclient.sendAsync(request, HttpResponse.BodyHandlers.ofString()).whenComplete((response, throwable) -> {
            if(context != null) {
                if(apiErrorHandler(context, throwable, response)) {
                    completableFuture.completeExceptionally(new Error("api error, handled via context"));
                    return;
                }
            } else {
    ...
    public static CompletableFuture<Wallet> createWallet(String name, UUID uuid) {
